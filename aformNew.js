...
/**
 * @param {{isReload:boolean}} data
 * @param {{reloadTimeout:string}} data
 * @param {{closePopup:string}} data
 */

Aform.pageReload = (data) => {

  if (data.isReload) {
    let to = 500
    if (data.reloadTimeout) {
      to = parseInt(data.reloadTimeout)
    }
    setTimeout(function () {
...
Aform.validateError = (formName, data) => {
